...
export function BettingCard({ site, rank }: BettingCardProps) {
  const getBadge = (rank: number) => {
    if (rank === 1) return { text: "TOP BRAND", color: "bg-green-500" }
    if (rank === 2) return { text: "EXCLUSIVE OFFER", color: "bg-green-500" }
    if (rank === 3) return { text: "TRENDING", color: "bg-green-500" }
    return { text: `#${rank}`, color: "bg-green-500" }
  }
  ...
  return (
    <a href={site.url} target="_blank" rel="noopener noreferrer" className="block">
      <div className="mb-3">
        ...
        <div className="hidden md:block bg-black text-white py-1 px-4 rounded-t-lg">
          ...
          <div className="p-3 relative">
            ...
            <div className="md:hidden">
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="text-2xl font-bold text-green-500">{site.rating}</div>
              </div>

              <div className="flex justify-center">
                <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-1.5 px-4 rounded-full text-sm transition-colors duration-300 w-full">
                  GET BONUS
                </button>
              </div>
            </div>
            ...
            <div className="hidden md:grid md:grid-cols-5 gap-2 items-center pt-2">
              ...
              {/* Score */}
              <div className="text-center">
                <div className="text-3xl font-bold text-green-500">{site.rating}</div>
              </div>
            </div>

            {/* Visit Site - Desktop only */}
            <div className="hidden md:flex md:justify-center mt-2">
              <button className="bg-green-500 hover:bg-green-600 text-white font-bold py-1.5 px-4 rounded-full text-sm transition-colors duration-300">
                GET BONUS
              </button>
            </div>
